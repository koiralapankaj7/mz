...
#### ControllerBuilder - Simplified Builder Signature

- Removed redundant `controller` parameter from builder callback
- Builder now uses `WidgetBuilder` typedef: `builder: (context) => ...`
- Controller is already available via the widget's `controller` property
...
ControllerBuilder<MyController>(
  controller: myController,
  builder: (context) => Text('${myController.value}'),
);
```

#### ControllerProvider - Optimized with Dual-Widget Pattern

- Default constructor now uses `StatelessWidget` (lightweight, no lifecycle management)
- New `autoDispose` static method for lifecycle management using `StatefulWidget`
- Follows ownership principle: "I created it, I dispose it"

**Simple provision (no dispose):**

```dart
// Uses StatelessWidget - minimal overhead
...
**Auto-dispose (creates + disposes):**

```dart
// Uses StatefulWidget - manages lifecycle
...
- `Controller.maybeOfType` now uses `getInheritedWidgetOfExactType` for `listen: false` case, which is more efficient than `findAncestorStateOfType`

## [Unreleased]

### Changed

- Migrated to monorepo structure at [koiralapankaj7/mz](https://github.com/koiralapankaj7/mz)
...
[1.4.0]: https://github.com/koiralapankaj7/mz/releases/tag/mz_core-v1.4.0
